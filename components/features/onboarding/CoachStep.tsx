...
export function CoachStep({ onNext, onBack }: CoachStepProps) {
  const coachPreset = useOnboarding((s) => s.coachPreset);
  const setCoachPreset = useOnboarding((s) => s.setCoachPreset);
  const coaches = CoachSystem.getAllCoaches();

  return (
    <OnboardingCard
      title="Pick your coach"
      description="You can switch anytime - this just sets the tone"
      footer={
    ...
    >
      ...
      <div className="space-y-3">
        {coaches.map(coach => (
          <button
            key={coach.preset}
            onClick={() => setCoachPreset(coach.preset)}
            className={`w-full p-4 rounded-lg border-2 transition-all text-left ${
              coachPreset === coach.preset
